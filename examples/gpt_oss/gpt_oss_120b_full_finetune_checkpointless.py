...
class TrainerBuilder:
    ...
    def _validate(self):
        ...
        context_parallel_size = self.cfg.strategy.context_parallel_size
        ...
        logger.info(f"World size: {world_size}, TP: {tensor_parallel_size}, PP: {pipeline_parallel_size}, CP: {context_parallel_size}, DP: {data_parallel_size}")
    ...
    def model(self):
        ...
        model = GPTOSSModel(model_config, tokenizer=tokenizer)
        return model
...
@hydra.main(version_base=None, config_path="./config/", config_name="gpt_oss_120b_pretrain_checkpointless.yaml")
def main(cfg):
    ...
    @HPWrapper(
        ...
        abort_timeout=60.0,
        checkpoint_manager=CheckpointManager(enable_offload=True),
