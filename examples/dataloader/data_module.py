...
class LLMDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        if stage == "fit" or stage is None:
            ...
            # For validation, we can use a subset of the training data
            # In a real scenario, you'd have a separate validation dataset
            # Here we just reuse the same dataset for simplicity, but
            # we could also setup a new dataset instead to show
            # how MMAP can hide both train/val dataset setup latency.
